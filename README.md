...
The reclassified files are the sweep files plus the three label columns. Text and values are byte-identical between the two folders, so if you keep only one folder, keep the reclassified one.
...
Every reported number is a function of these files. The response classes come from the rubric in `battery_v1.json` applied to the `text` column; the AI reference figures come from the three patterns printed in the paper's Appendix B; the corpus tables come from `corpus_counts.csv`; the block 3
